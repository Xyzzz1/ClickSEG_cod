...
def evaluate_dataset(dataset, predictor, vis=True, vis_path='./experiments/vis_val/', **kwargs):
    ...
    for index in tqdm(range(len(dataset)), leave=False):
        sample, name = dataset.get_sample(index)
...
def evaluate_sample(image, gt_mask, init_mask, predictor, max_iou_thr,
                    ...
                    sample_id=None, vis=True, save_dir=None, index=0, name=None, callback=None,
    ...
    with torch.no_grad():
        ...
        if vis:
            if predictor.focus_roi is not None:
                ...
                cv2.imwrite(save_dir + str(name) + '.png', out_image)
                cv2.imwrite(save_dir + str(name) + '_focus.png', focus_image)
                ...
                cv2.imwrite(save_dir + 'vis/' + str(name) + '.png', out_image)

                # save mask only
                pred_mask = pred_mask.astype(np.float32) * 255
                cv2.imwrite(save_dir + 'result/' + str(name) + '.png', pred_mask)
